...
class Environment():
    ...
    def update_physical_state(self, org_id, nodes, edges, cs_areas, physical_state, material):        
        ...
        tension_coeffs = self._cal_tension_coeffs(equilibrium_mat, self.loads)
        ...
        #physical_state.strains = self._cal_member_strains(physical_state.stresses, material.young_mods)
        physical_state.strain_energies = self._cal_member_strain_energies(physical_state.stresses,
                                                                        physical_state.volumes, material.young_mods)
        #physical_state.node_disps = self._cal_joint_displacements(nodes, mem_lens, self.reactions, equilibrium_mat, physical_state.strains, dim=2)
        #physical_state.node_disps = self._cal_joint_displacements2(nodes, mem_lens, self.reactions, equilibrium_mat, physical_state.strains, node_constraints=[0,2,4,6,8], dim=2)
    ...
    def _cal_joint_displacements2(self,
                                 nodes,
                                 mem_lens,
                                 reactions,
                                 equilibrium_mat,
        ...
        num_dofs = num_nodes * dim  # 2 DOF per node
        free_dofs = []
        free_indices = []

        # Build list of free DOFs (skip constrained nodes)
        for n in range(num_nodes):
            if n in node_constraints:
                continue
            for c in range(dim):  # 0 = x, 1 = y
                free_dofs.append(n * dim + c)
                free_indices.append((n, c))

        joint_disp = np.zeros((num_nodes, dim))

        for g_dof, (n_idx, comp) in zip(free_dofs, free_indices):
            # Create unit virtual load
            virtual_load = np.zeros(num_dofs)
            virtual_load[g_dof] = 1.0

            # Solve for virtual internal forces
            virt_t = self._cal_tension_coeffs(equilibrium_mat, virtual_load)
            virt_N = self._cal_member_forces(virt_t, reactions, mem_lens)

            disp = np.sum(virt_N * mem_deforms)
            joint_disp[n_idx, comp] = disp

        mask = np.ones(num_nodes, dtype=bool)
        mask[node_constraints] = False
        return joint_disp[mask]  # shape: (n_free_nodes, dim)
